module PickWordInSetsHelper
  def new_pick_word_in_set_url_from_names(target, display, option, source)
    ...
    url = new_pick_word_in_set_url(params: {n: 10, target: target, \
      display: display, option: option, source: source})
    display = display.kind_of?(Array) ? display.map{|d|d.capitalize}.to_s : display.capitalize
    cap = "(#{source.capitalize} -> #{target.capitalize}): [#{display} >>> #{option.capitalize}]"
    link_to(cap, url)
  end
end
