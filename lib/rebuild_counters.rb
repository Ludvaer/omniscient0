# UserWordLearnProgress.delete_all
...
#       lp = UserWordLearnProgress.find_or_create_by!(user_id:user.id,translation_id: t_id)
...
[[User.yarxi_seed,User.yarxi_combo_seed],[User.jishop_seed,User.jishop_combo_seed],[User.jmdict_parser]].each do |users|
  Dialect.all.to_a.product(Dialect.all.to_a).each do  |source_dialect,target_dialect,user|
    puts "#{users.map(&:name)}: #{source_dialect.name} -> #{target_dialect.name}"
    translations =
        Translation.joins(:word)
        .where('word.dialect_id':target_dialect.id, translation_dialect_id:source_dialect.id, user_id:users.map(&:id))
        .order(:priority)
