...
level_folders.each_with_index  do |level_folder, level|
  ...
  csv.each do |row|
    ...
    word = [(row[:kanji].nil? || row[:kanji].strip.empty?) ? row[:reading].strip : row[:kanji].strip, row[:reading].strip,
     priority,
     priority +csv.length]
    word[1] = word[0] if word[0].kana_with_symbol?
    word[1]  = word[1].safe_hiragana if word[1].katakana_with_symbol?
    puts "non kana reading in levels #{word[1]}" unless word[1].kana_with_symbol?
    # puts word
...
frequent_words.map{|w|w.gsub('（',' ').gsub('）','').split('、')}.flatten.each do |line|
  ...
  if line.include?('【')
    ...
    kana = kanji if kanji.kana_with_symbol? #I think some kana might be broken for kana only words
  else
    kanji = line
  end
  kana = kana.safe_hiragana if kana.katakana_with_symbol?
  puts "non kana reading in frequncy #{kana}" unless kana.kana_with_symbol?
    ...
    unless kana
      ...
      if translation.nil? and not kanji.kana_with_symbol? #search similar word if it's not juist kana
        translation = Translation.eager_load(:word)\
          ...
          .order("CHAR_LENGTH(word.spelling)").first
